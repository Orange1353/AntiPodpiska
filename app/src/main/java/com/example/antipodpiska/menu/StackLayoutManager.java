...
import com.example.antipodpiska.BuildConfig;

import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;

import static androidx.recyclerview.widget.RecyclerView.NO_POSITION;
import static com.example.antipodpiska.Align.BOTTOM;
import static com.example.antipodpiska.Align.LEFT;
import static com.example.antipodpiska.Align.RIGHT;
import static com.example.antipodpiska.Align.TOP;
...
class StackLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //Менять расстояние между сабами  (-100)
        if (canScrollHorizontally())
            mUnit = mItemWidth + mSpace-getWidth()/14;
        else mUnit = mItemHeight + mSpace-getWidth()/14;
        ...
        fill(recycler, 0);
        if(countScroll == 0) {
            scrollToPosition(getChildCount());
        countScroll++;
        }
    }
...
//            if(i == end)
 //           layoutDecoratedWithMargins(view, left, top+10*i+50, right, bottom);
//else
          ;
//    layoutDecoratedWithMargins(view, left,top +(10+getWidth()/10*i), right, bottom+10+getWidth()/10*i);
           layoutDecoratedWithMargins(view, left, top, right, bottom);

            view.setAlpha(1.0f);
            if (alpha <= 0.2)
                view.setAlpha(alpha*4);
