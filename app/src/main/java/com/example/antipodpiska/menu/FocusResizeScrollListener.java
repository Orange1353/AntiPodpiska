...
public class FocusResizeScrollListener<T extends com.example.library.FocusResizeAdapter> extends RecyclerView.OnScrollListener {
    ...
    private void calculateScrolledPosition(int totalItemCount, RecyclerView recyclerView) {
        for (int j = 0; j < totalItemCount - 1; j++) {
            ...
            if (view != null) {
                if (!(recyclerView.getChildViewHolder(view) instanceof com.example.library.FocusResizeAdapter.FooterViewHolder)) {
                    if (j == itemToResize) {
